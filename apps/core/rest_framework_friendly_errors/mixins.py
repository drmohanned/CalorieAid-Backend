...
class FriendlyErrorMessagesMixin(FieldMap):
    ...
    def get_field_kwargs(self, field, field_data):
        ...
        if field_type in self.field_map['boolean']:
            ...
            kwargs.update({'input_type': type(field_data).__name__,
                           ...
                           'min_length': getattr(field, 'min_length', None)})
        elif field_type in self.field_map['nested_serializer']:
            kwargs.update({'input_type': type(field_data)})
            kwargs.update({'datatype': type(field_data)})
    ...
    def _run_validator(self, validator, field, message):
        ...
        except KeyError:  # nested serializers
            field_name = field.field_name.split('.')[0]
            validator(self.initial_data[field_name])
    ...
    def get_field_error_entry(self, error, field, field_name=None):
        ...
        if not key:
            ...
            except AttributeError:  # nested serializers
                field_original_name = field.field_name.split('.')[0]
                validator = getattr(self, "validate_%s" % field_original_name)
            if self._run_validator(validator, field, error):
                name = validator.__name__
                code = self.FIELD_VALIDATION_ERRORS.get(name) or settings.FRIENDLY_VALIDATOR_ERRORS.get(name)
                return {'code': code, 'field': field_name,
                        'message': error}
    ...
    def get_field_error_entries(self, errors, field, field_name=None):
        return [self.get_field_error_entry(error, field, field_name) for error in errors]
    ...
    def serialize_complex_errors(self, errors, parent_field_name='', serialized_errors={}):
        if isinstance(errors, dict):
            for field_name, error in errors.items():
                for err in error:
                    if parent_field_name:
                        field = parent_field_name + '.' + field_name
                    else:
                        field = field_name
                    if isinstance(err, dict):
                        self.serialize_complex_errors(err, field, serialized_errors)
                    else:
                        serialized_errors[field] = error
    ...
    def build_pretty_errors(self, errors):
        ...
        for error_type in errors:
            if error_type == 'non_field_errors':
                ...
                if isinstance(errors[error_type], dict):  # nested serializers
                    for nested_field_name in errors[error_type]:
                        temp_field = copy(field)
                        err_msg = errors[error_type][nested_field_name]
                        temp_field.field_name = temp_field.field_name + '.' + nested_field_name


                        if isinstance(err_msg, dict):
                            serialized_errors = self.serialize_complex_errors(err_msg)
                        elif isinstance(err_msg, list) and isinstance(err_msg[0], dict):
                            for err in err_msg:
                                temp_field = copy(field)
                                temp_field.field_name = temp_field.field_name + '.' + list(err.keys())[0]
                                serialized_errors = self.serialize_complex_errors(err)

                                for field_name, serialized_error in serialized_errors.items():
                                    pretty.extend(
                                        self.get_field_error_entries(serialized_error, temp_field,
                                                                     field_name=field_name),
                                    )

                        else:
                            serialized_errors = errors[error_type]

                            for field_name, serialized_error in serialized_errors.items():
                                pretty.extend(
                                    self.get_field_error_entries(serialized_error, temp_field, field_name=field_name),
                                )

                elif isinstance(errors[error_type], list) and isinstance(errors[error_type][0], dict):  # nested serializers , with many=True param
